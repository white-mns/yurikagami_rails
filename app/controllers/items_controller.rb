class ItemsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= Item.notnil().includes(:pc_name, :kind, :effect, :ability).search(params[:q]).result.hit_count()
    @search	= Item.notnil().includes(:pc_name, :kind, :effect, :ability).page(params[:page]).search(params[:q])
