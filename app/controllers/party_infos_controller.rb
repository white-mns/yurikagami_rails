class PartyInfosController < ApplicationController
  ...
  def index
    param_set
    @count	= PartyInfo.includes(party_members: :p_name).ransack(params[:q]).result.count()
    @search	= PartyInfo.includes(party_members: :p_name).page(params[:page]).ransack(params[:q])
