class LearnableSkillsController < ApplicationController
  ...
  def index
    placeholder_set
    param_set
    @count	= LearnableSkill.includes(:job, [skill: [:timing, :target, :property, :element]]).ransack(params[:q]).result.hit_count()
    @search	= LearnableSkill.includes(:job, [skill: [:timing, :target, :property, :element]]).page(params[:page]).ransack(params[:q])
