class EnemyPartyInfosController < ApplicationController
  ...
  def index
    param_set
    @count	= EnemyPartyInfo.includes(enemy_members: :enemy_name, current_place: [:place_name, party_info: [party_members: :p_name]]).group(:party_no, :result_no).ransack(params[:q]).result.count().keys().size
    @search	= EnemyPartyInfo.includes(enemy_members: :enemy_name, current_place: [:place_name, party_info: [party_members: :p_name]]).group(:party_no, :result_no).page(params[:page]).ransack(params[:q])
