class CurrentPlace < ApplicationRecord
    ...
    scope :where_start, ->() {
        ...
        self.includes(:party_info).where(place_id: [place_id["港町キュースター"], place_id["貿易都市オゴト"], place_id["応礁"]]).where("party_infos.battler_num = 0")
    }

    scope :to_start_graph, ->(column) {
        place_name = Hash[*ProperName.pluck(:proper_id, :name).flatten]
        place_country = {"港町キュースター" => "リバーシ", "貿易都市オゴト" => "キシェタトル", "応礁" => "平須"}
